...
class ClassificationAgent():
    ...
    def get_response(self , messages):
        ...
        system_prompt = """
        You are a helpful AI assistant for a coffee shop application.
        Your task is to determine which agent should handle the user's input. You have 3 agents to choose from:

        1. details_agent: This agent is responsible for answering questions about the coffee shop, such as location, delivery areas, working hours, details about menu items, listing menu items, or responding to questions like "What do you have?"

        2. order_taking_agent: This agent is responsible for taking orders from the user. It handles conversations about the order until it is complete.

        3. recommendation_agent: This agent is responsible for giving recommendations to the user about what to buy. If the user asks for a recommendation, this agent should be assigned.

        Your output should be in the following structured JSON format. Each key and value must be a string. Make sure to follow the format exactly:

        {
            "chain of thought": "Review each of the agents above and provide your reasoning for which agent is most relevant to the user's input.",
